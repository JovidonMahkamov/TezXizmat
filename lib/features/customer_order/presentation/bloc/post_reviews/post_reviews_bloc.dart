...
class PostReviewsBloc extends Bloc<CustomerOrderEvent, PostReviewsState> {
  final PostReviewsUseCase postReviewsUseCase;

  PostReviewsBloc(this.postReviewsUseCase) : super(PostReviewsInitial()) {
    on<PostReviewsE>(onLogInUser);
  }

  Future<void> onLogInUser(event, emit) async {
    emit(PostReviewsLoading());
    try {
      final result = await postReviewsUseCase(
        orderId: event.orderId,
        stars: event.stars,
        text: event.text,
      );
      emit(PostReviewsSuccess(postReviewsEntity: result));
    } on DioException catch (e) {
      String errorMessage = _mapDioErrorToMessage(e);
      emit(PostReviewsError(message: errorMessage));
    } catch (e) {
      emit(PostReviewsError(message: "Noma’lum xato yuz berdi"));
    }
  }
