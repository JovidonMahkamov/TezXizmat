...
class WorkerRemoteDataSourceImpl implements WorkerRemoteDataSource {
  ...
  WorkerRemoteDataSourceImpl(this.customerDioClient,
      this.staffDioClient,
      this.local,);
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        LoggerService.warning(
          "get worker profile failed: ${response.statusCode}",
        );
  ...
  Future<WorkerEditProfileModel> editProfile({
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        LoggerService.warning(
          "edit worker profile failed: ${response.statusCode}",
        );
  ...
  Future<WorkerProfileImageModel> putImage({required String filePath}) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        LoggerService.info(
          'update worker profile image successful: ${response.data}',
        );
        return WorkerProfileImageModel.fromJson(response.data);
      } else {
        LoggerService.warning(
          "update worker profile image failed: ${response.statusCode}",
        );
        throw Exception(
          'update worker profile image failed: ${response.statusCode}',
        );
  ...
  @override
  Future<List<MyReviewsModel>> getMyReviews() async{
    try {
      final response = await customerDioClient.get(
        StaffApiUrls.getMyReviews,
      );
      if (response.statusCode == 200 || response.statusCode == 201) {
        LoggerService.info('get my reviews successful: ${response.data}');
        final data = response.data;
        final List list = data is List ? data : (data['results'] as List);

        return list
            .map((e) => MyReviewsModel.fromJson(e as Map<String, dynamic>))
            .toList();
      } else {
        LoggerService.warning(
          "get my reviews failed: ${response.statusCode}",
        );
        throw Exception('get my reviews failed: ${response.statusCode}');
      }
    } catch (e, s) {
      LoggerService.error('Error during get my reviews: $e');
      print(e);
      print(s);
      rethrow;
    }
